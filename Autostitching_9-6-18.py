...
for file in os.listdir('/home/sinadabiri/Dropbox/Images/cell/'):
    if file.endswith(".tif"): 

    
    	path = str(os.path.join('/home/sinadabiri/Dropbox/Images/cell/', file))
    	
    	img[i] = cv2.imread(path, -1)
    	print (img[i])
    	i+=1
        continue
    else:
...
overLapCorrCoef= np.corrcoef(img[1][:,(col-1):1:-1], img[2][:,1:(col2-1)], rowvar=False)[:,(col-1)]

overLapCorrCoefVertical= np.corrcoef(img[1][(row-1):1:-1,:], img[2][1:(row2-1),:], rowvar=True)[(row-1),:]

deltaX= np.argmax(overLapCorrCoef[1:col/2]) 
print(col/2, overLapCorrCoef[1:col/2])
print (deltaX,np.amax(overLapCorrCoef[1:col/2])) 

deltaY = np.argmax(overLapCorrCoefVertical[1:row/2]) 
print(row/2, overLapCorrCoefVertical[1:row/2])
print (deltaY,np.amax(overLapCorrCoefVertical[1:row/2])) 

M = np.float32([[1,0,-(deltaY-1)],[0,1,-(deltaX-1)]])
img2_translated = cv2.warpAffine(img[2],M,(col2,row2))
cv2.imwrite('/home/sinadabiri/Dropbox/Images/cell3Translated.tif',img2_translated)
...
def PixelAveraging(img1,img2, pixelNumbersToAverage,pixelincrement):
	...
	for i in range (row):
		if i<(row-1): 
			...
			for j in range(col):
				if j<(col-1):
					img1_Avg[i:i+pixelincrement,j:j+pixelincrement]=np.average([img[1][i:i+pixelincrement,j:j+pixelincrement]])
					img2_Avg[i:i+pixelincrement,j:j+pixelincrement]=np.average([img[2][i:i+pixelincrement,j:j+pixelincrement]])
...
PixelAveraging(img[1],img[2],pixelNumbersToAverage,pixelincrement)
...
def OverLayFFT(img1,img2):
	...
	# Calculating 2D FFT
	img1FFT2 = np.fft.fft2(img[1])
	img2FFT2 = np.fft.fft2(img[2])
	...
	img1DFT = cv2.dft(np.float32(img[1]), flags=cv2.DFT_COMPLEX_OUTPUT)
	img2DFT = cv2.dft(np.float32(img[2]), flags=cv2.DFT_COMPLEX_OUTPUT)
...
plt.subplot(321),plt.imshow(img[1],cmap = 'gray'),plt.title('Image 1')
plt.xticks(),plt.yticks([])
plt.subplot(323),plt.imshow(img[2],cmap = 'gray'), plt.title('Image 2')
plt.xticks(),plt.yticks([])
...
plt.xticks(),plt.yticks([])
plt.subplot(324),plt.imshow(img[2],cmap = 'gray'), plt.title('Image 2 original')
plt.xticks(),plt.yticks([])
...
# plt.title('img[2]-img[1] FFT phase difference spectrum'), plt.xticks(),plt.yticks([])
