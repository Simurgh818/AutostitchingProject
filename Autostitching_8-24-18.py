...
for file in os.listdir('/home/sinadabiri/Dropbox/Images/robo6_2x2_103pixelOverlap/'):
    if file.endswith(".tif"): 
    	...
    	print(os.path.join('/home/sinadabiri/Dropbox/Images/robo6_2x2_103pixelOverlap/', file))
    	img[i] = cv2.imread(str(os.path.join('/home/sinadabiri/Dropbox/Images/robo6_2x2_103pixelOverlap/', file)), -1)
    	i+=1
        continue
    else:
        continue
...
overLapCorrCoefVertical= np.corrcoef(img1[(row-1):1:-1,:], img2[1:(row2-1),:], rowvar=True)[(row-1),:]
...
img2_translated = cv2.warpAffine(img2,M,(col2,row2))
...
plt.xticks(),plt.yticks([])
plt.subplot(324),plt.imshow(img2,cmap = 'gray'), plt.title('Image 2 original')
plt.xticks(),plt.yticks([])
...
# plt.title('img2-img1 FFT phase difference spectrum'), plt.xticks(),plt.yticks([])
# plt.subplot(334),plt.imshow(img2Corrected,cmap = 'gray')
# plt.title('img2 phase corrected'), plt.xticks(),plt.yticks([])
